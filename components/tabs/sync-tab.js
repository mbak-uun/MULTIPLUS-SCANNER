...
const SyncTab = {
  ...
  methods: {
    ...
    formatPrice(value) {
      if (value === undefined || value === null || value === '') return '-';
      const num = Number(value);
      if (Number.isNaN(num)) return String(value); // Kembalikan string asli jika bukan angka
    ...
    async initialize() {
      ...
      try {
        // Langkah 1: Muat cache dari DB untuk memeriksa statusnya.
        await this.loadCacheFromDB();

        // Langkah 2: Jika cache kosong untuk CEX yang aktif, panggil auto-fetch.
        // Fungsi ini akan mengisi cache jika perlu.
        await this.ensureCacheAvailability();

        // Langkah 3: Setelah cache dipastikan terisi (baik dari DB atau auto-fetch),
        // perbarui tampilan.
    ...
    async ensureCacheAvailability() {
      ...
      console.log('[ensureCacheAvailability] Checking CEXs without data:', cexWithoutData);
      console.log('[ensureCacheAvailability] Current cexDataStatus:', this.cexDataStatus);

      if (cexWithoutData.length === 0) {
        console.log('[ensureCacheAvailability] Semua CEX sudah memiliki data, skip auto-fetch.');
        return;
      }

      this.$root.isLoading = true;
      this.$root.loadingText = `Cache kosong, mengambil data awal dari server...`;
      console.log(`[ensureCacheAvailability] 🔄 Memulai auto-fetch untuk ${cexWithoutData.length} CEX...`);
      ...
      try {
        ...
        console.log(`[ensureCacheAvailability] ✅ JSON berhasil diambil, total records:`, remoteData.length);

        for (const cex of cexWithoutData) {
          try {
            const upperCex = this.normalizeCex(cex);
            console.log(`[ensureCacheAvailability] Processing ${upperCex}...`);
            ...
            console.log(`[ensureCacheAvailability] Found ${dataForCex.length} records for ${upperCex}`);
            ...
            if (dataForCex.length > 0) {
              ...
              successCount++;
              console.log(`[ensureCacheAvailability] ✅ ${upperCex}: ${dataForCex.length} koin disimpan ke cache`);
              this.$emit('show-toast', `✓ ${upperCex}: ${dataForCex.length} koin berhasil dimuat dari JSON`, 'success');
            } else {
              console.warn(`[ensureCacheAvailability] ⚠️ ${upperCex}: Tidak ada data di JSON untuk CEX ini`);
            }
          } catch (cexError) {
            errorCount++;
            console.error(`[ensureCacheAvailability] ❌ Error processing ${cex}:`, cexError);
            this.$emit('show-toast', `✗ Auto-fetch ${cex}: ${cexError.message}`, 'danger');
          }
        }

        if (successCount > 0) {
          const message = `Auto-fetch selesai: ${successCount} CEX berhasil, ${errorCount} gagal.`;
          this.$emit('show-toast', message, successCount > errorCount ? 'success' : 'warning');
          // REVISI KRUSIAL: Setelah auto-fetch berhasil, muat ulang cache dari DB
          // agar `this.syncCache` dan `this.cexDataStatus` terisi dengan data baru.
          await this.loadCacheFromDB();
        }
      } catch (error) {
    ...
    _normalizeJsonToken(item, cex) {
      ...
      // PERBAIKAN: Struktur JSON baru
      // - item.nama_token = Nama lengkap (e.g., "Towns Protocol")
      // - item.ticker = Ticker/symbol untuk trading (e.g., "TOWNS")
      // - item.sc = Smart contract address
      // - item.decimals = Decimals

      const ticker = String(item.ticker || item.symbol || '').toUpperCase();
      const namaLengkap = String(item.nama_token || item.name || item.nama_koin || '');

      const normalized = {
        cex,
        chain: String(chain),
        nama_koin: namaLengkap,           // Nama lengkap: "Towns Protocol"
        nama_token: ticker,               // Ticker: "TOWNS"
        cex_ticker: this._formatCexTicker(cex, ticker), // Format untuk CEX: "TOWNSUSDT"
        sc_token: String(item.sc || item.contract || item.address || ''),
        des_token: Number(item.decimals ?? item.des ?? 0),
    ...
    async updateSyncDataView() {
      ...
      for (const cex of this.selectedCexFilters) {
        // PERBAIKAN: Normalize key untuk memastikan konsistensi
        const normalizedCex = this.normalizeCex(cex);

        if (Array.isArray(this.syncCache[normalizedCex])) {
          // Filter koin dari cache
          // REVISI: Hapus filter sc_token di sini agar SEMUA data dari cache ditampilkan.
          // Filter sc_token sudah diterapkan pada saat penghitungan badge count.
          // Dengan menampilkan semua, pengguna bisa melihat data apa saja yang berhasil di-sync,
    ...
    async toggleCexSelection(cex) {
      ...
      const wasSelected = this.isCexSelected(upper);
      ...
      // PERBAIKAN: Jika CEX baru dicentang dan cache-nya kosong, auto-fetch dari JSON
      if (!wasSelected) {
