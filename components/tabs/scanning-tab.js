...
const TokenRow = {
  ...
  methods: {
    ...
    formatDexCell(dexKey, direction) {
      ...
      const formatterOptions = {
        ...
        modalUsd: tokenModal, // Gunakan modal dari database
  ...
  template: `
    <tr class="token-row">
      ...
      <td v-for="dex in activeDexList" :key="'left-' + dex.key + '-' + token.id" class="text-center dex-cell">
        <div
          v-if="isDexActive(dex.key)"
          :id="getSignalCellId(dex.key, 'CEXtoDEX')"
          v-html="formatDexCell(dex.key, 'CEXtoDEX')"
        ></div>
      ...
      <td v-for="dex in activeDexList" :key="'right-' + dex.key + '-' + token.id" class="text-center dex-cell">
        <div
          v-if="isDexActive(dex.key)"
          :id="getSignalCellId(dex.key, 'DEXtoCEX')"
          v-html="formatDexCell(dex.key, 'DEXtoCEX')"
        ></div>
        <div v-else class="small text-muted">-</div>
      </td>
      <td class="text-center orderbook-cell">
...
const ScanningTab = {
  ...
  computed: {
    ...
    // MULTICHAIN MODE: Check if we're in multichain mode
    isMultiChainMode() {
    ...
    <div class="scanning-tab">
      ...
      <div class="row g-2 mb-3" v-if="activeDexList.length > 0">
        <div v-for="dex in activeDexList" :key="dex.key" class="col-6 col-md-4 col-lg-3 col-xl">
          <!-- REFACTORED: Background seragam dengan chain theme, warna DEX untuk teks -->
          <div class="card h-100 shadow-sm">
            <div class="card-header p-2" :style="getDexHeaderStyles(dex.key)">
              <h6 class="card-title text-center mb-0 fw-bold" :style="$root.getColorStyles('dex', dex.key, 'text')">
                {{ dex.name }}
              </h6>
            </div>
            <div class="card-body p-2">
              <div v-if="getSignalsForDex(dex.key).length > 0">
                <button
                  v-for="signal in getSignalsForDex(dex.key)"
                  :key="signal.id"
                  type="button"
                  class="signal-item btn w-100 text-start mb-1 p-2 rounded border"
                  :class="[
                    signal.meetsThreshold ? 'signal-item-success' : 'signal-item-muted text-light',
                    signal.direction === 'CEXtoDEX' ? 'signal-direction-positive' : 'signal-direction-negative'
                  ]"
                  @click="goToSignal(signal)"
                >
                  <span class="signal-bullet">🔸</span>
                  <span
                    class="signal-cex fw-semibold me-1"
                    :style="$root.getColorStyles('cex', signal.cexKey, 'text')"
                  >
                    {{ signal.cexName }}
                  </span>
                  <span class="signal-modal me-1">:{{ signal.modalDisplay }}</span>
                  <span :class="['signal-direction fw-semibold me-1', signal.directionClass]">
                    {{ signal.tokenIn }}->{{ signal.tokenOut }}
                  </span>
                  <span class="signal-pnl text-warning fw-semibold">{{ signal.pnlUsdFormatted }}</span>
                </button>
              </div>
      ...
      <div class="table-responsive " style="max-height: calc(100vh - 300px);">
        <table class="table table-sm table-hover table-striped-columns align-middle scanning-table">
