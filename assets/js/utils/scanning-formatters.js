...
const ScanningFormatters = {
  ...
  formatDexCell(pnlData, token, direction, options = {}) {
    ...
    if (pnlData === null || pnlData === undefined) {
      ...
      const modalDisplay = modalUsd > 0
        ? `<div class="text-muted small fw-bold mb-1">${formatUsd(modalUsd)}</div>`
        : '';
    ...
    const modalDisplay = (pnlData.modal || modalUsd) > 0
      ? `<div class="text-muted small fw-bold mb-1">${formatUsd(pnlData.modal || modalUsd)}</div>`
      : '';
