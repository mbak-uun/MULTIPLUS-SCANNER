...
class PriceScanner {
    ...
    async _processToken(token, filters, gasData, usdtRate) {
        ...
        try {
            ...
            if (cexKey) {
                ...
                const tokenOrderbookPromise = (async () => {
                    if (!tokenSymbol) return null;
                    const orderbook = await this.cexFetcher.getOrderbook(cexKey, tokenSymbol);
                    await this.delayManager.waitCexDelay(cexKey);
                    return orderbook;
                })();

                const pairOrderbookPromise = (async () => {
                    if (!pairSymbol) return null;
                    await this.delayManager.waitCexDelay(cexKey);
                    const orderbook = await this.cexFetcher.getOrderbook(cexKey, pairSymbol);
                    await this.delayManager.waitCexDelay(cexKey);
                    return orderbook;
                })();

                const [tokenOrderbook, pairOrderbook] = await Promise.all([
                    tokenOrderbookPromise,
                    pairOrderbookPromise
                ]);

                cexPrices.token = tokenOrderbook;
                cexPrices.pair = pairOrderbook;
            }
            ...
            const dexJobs = activeDexKeys.map(async (dexKey, dexIndex) => {
                ...
                try {
                    const modalConfig = this._resolveDexModalUsd(token, dexKey);
                    const dexInput = this._calculateDexInputAmounts(token, cexPrices, modalConfig);

                    if ((!dexInput.tokenToPair || dexInput.tokenToPair <= 0) &&
                        (!dexInput.pairToToken || dexInput.pairToToken <= 0)) {
                        const errorResult = {
                            cexToDex: { error: true, errorMessage: 'Modal DEX tidak valid' },
                            dexToCex: { error: true, errorMessage: 'Modal DEX tidak valid' }
                        };
                        pnlResults[dexKey] = errorResult;
                    ...
                    if (quotes) {
                        ...
                        const pnlCexToDex = this.pnlCalculator.calculateBothDirections({
                            ...
                            direction: 'CEXtoDEX',
                            modalUsd: modalConfig.cexToDexUsd,
                            gasData,
                            usdtRate
                        });
                        ...
                        const pnlDexToCex = this.pnlCalculator.calculateBothDirections({
                            ...
                            direction: 'DEXtoCEX',
                            modalUsd: modalConfig.dexToCexUsd,
                            gasData,
                            usdtRate
                        });
    ...
    /**
     * Tentukan modal USD yang akan digunakan per DEX untuk kedua arah
     * @private
     */
    _resolveDexModalUsd(token, dexKey) {
        const parseModal = (value) => {
            const numeric = Number(value);
            return Number.isFinite(numeric) && numeric > 0 ? numeric : null;
        };

        const dexConfig = token?.dex?.[dexKey] || {};
        const directionLeft = parseModal(dexConfig.left);
        const directionRight = parseModal(dexConfig.right);

        const tokenLevel = parseModal(token?.modalUsd) ?? parseModal(token?.modal);
        const globalLevel = parseModal(this.settings.modalUsd);
        const fallback = tokenLevel ?? globalLevel ?? 100;

        return {
            cexToDexUsd: directionLeft ?? fallback,
            dexToCexUsd: directionRight ?? fallback
        };
    }
     ...
     * @private
     */
    _calculateDexInputAmounts(token, cexPrices, modalConfig = {}) {
        const parseModal = (value, fallback = null) => {
            const numeric = Number(value);
            if (Number.isFinite(numeric) && numeric > 0) {
                return numeric;
            }
            if (fallback !== null) {
                return parseModal(fallback, null);
            }
            return 0;
        };

        const globalModal = parseModal(this.settings.modalUsd || 0) || 0;
        const modalUsdToken = parseModal(modalConfig.cexToDexUsd, globalModal) || globalModal;
        const modalUsdPair = parseModal(modalConfig.dexToCexUsd, globalModal) || globalModal;
        ...
        const tokenAmountBought = buyPriceToken > 0 ? (modalUsdToken / buyPriceToken) : 0;
        ...
        const pairAmountBought = buyPricePair > 0 ? (modalUsdPair / buyPricePair) : 0;
        ...
        return {
            ...
            modalCexToDexUsd: modalUsdToken,
            modalDexToCexUsd: modalUsdPair
        };
    }
