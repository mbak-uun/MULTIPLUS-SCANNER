...
class DexDataFetcher {
    ...
    async _fetchSingleQuote(dexKey, token, inputAmounts, direction, globalSettings) {
        ...
        try {
            ...
            // OPTIMIZATION: Use CentralizedApiService if available and caching is enabled
            let response;
            if (this.enableCache && typeof window.CentralizedApiService !== 'undefined') {
                response = await window.CentralizedApiService.fetch({
                    service: `dex-${dexKey.toLowerCase()}-${strategyKey}`,
                    endpoint: 'quote',
                    params: {
    ...
    setCacheEnabled(enabled) {
        ...
        logger.log(`[DexDataFetcher] Caching ${enabled ? 'enabled' : 'disabled'}`);
    }

    /**
     * OPTIMIZATION: Clear all cached quotes
     */
    clearCache() {
        if (typeof window.CentralizedApiService !== 'undefined') {
            window.CentralizedApiService.clearCache(/^dex-/);
            logger.log('[DexDataFetcher] DEX quote cache cleared');
        }
    }

    /**
     * OPTIMIZATION: Clear cache for specific DEX
     * @param {string} dexKey - DEX key (e.g., 'odos', 'kyber')
     */
    clearDexCache(dexKey) {
        if (typeof window.CentralizedApiService !== 'undefined') {
            const pattern = new RegExp(`^dex-${dexKey.toLowerCase()}-`);
            window.CentralizedApiService.clearCache(pattern);
            logger.log(`[DexDataFetcher] Cache cleared for DEX: ${dexKey}`);
        }
    }
}
