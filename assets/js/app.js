...
const app = createApp({
  ...
  computed: {
    ...
    pairList() {
      // REVISI: Sumber data diubah menjadi `this.allCoins` agar hanya menampilkan pair yang benar-benar digunakan.
      const groupedPairs = {};
      const uniquePairs = new Set();

      // Kumpulkan semua pair unik dari data koin yang sudah dimuat
      this.allCoins.forEach(coin => {
        if (coin.id === 'DATA_KOIN' || !coin.chain || !coin.nama_pair) return;
        const chainKey = coin.chain.toLowerCase();
        const pairSymbol = coin.nama_pair.toUpperCase();
        if (pairSymbol !== 'NON') { // Jangan tampilkan 'NON' sebagai opsi filter
          const uniquePairKey = `${chainKey}.${pairSymbol}`;
          if (!groupedPairs[chainKey]) {
            groupedPairs[chainKey] = [];
          }
          // Pastikan tidak ada duplikat dalam satu chain
          if (!groupedPairs[chainKey].includes(uniquePairKey)) {
            groupedPairs[chainKey].push(uniquePairKey);
          }
        }
      });
