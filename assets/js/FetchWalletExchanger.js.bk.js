...
class CheckWalletExchanger {
  ...
  async fetchTradeStatus(selectedCex = []) {
    const tradeSources = {
      ...
      MEXC: { url: 'https://api.mexc.com/api/v3/exchangeInfo', useProxy: false, parse: r => new Set((r?.symbols || []).filter(it => String(it.status || '').toUpperCase().includes('EN')).filter(it => (it.quoteAsset === 'USDT' || it.quoteAsset === 'USD')).map(it => String(it.baseAsset || '').toUpperCase())) },
  ...
  async fetchPrices(selectedCex = []) {
    ...
    const priceSources = {
      ...
      MEXC: { url: 'https://api.mexc.com/api/v3/ticker/price', useProxy: false, parse: r => buildPriceMap(r, 'symbol', 'price', /USDT$/) },
        ...
        items.forEach(item => {
          (item.networkList || []).forEach(net => {
            ...
            out.push({
              cex: 'BINANCE',
              chain: chainKey,
              token: item.assetName || item.coin || item.asset || '',
              symbol: String(item.coin || item.asset || '').toUpperCase(),
              sc: net.contractAddress || net.address || '',
        ...
        (res || []).forEach(item => {
          (item.chains || []).forEach(ch => {
            ...
            out.push({
              cex: 'GATE',
              chain: chainKey,
              token: item.name || item.currency || '',
              symbol: String(item.currency || '').toUpperCase(),
              sc: ch.addr || '',
        ...
        (res?.data || []).forEach(item => {
          (item.chains || []).forEach(ch => {
            ...
            out.push({
              cex: 'BITGET',
              chain: chainKey,
              token: item.name || item.coin || '',
              symbol: String(item.coin || '').toUpperCase(),
              sc: ch.contractAddress || '',
