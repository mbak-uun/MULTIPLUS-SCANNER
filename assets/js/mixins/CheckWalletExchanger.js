...
class CheckWalletExchanger {
  ...
  async fetchTradeStatus(selectedCex = []) {
    const tradeSources = {
      ...
      BITGET: { url: 'https://api.bitget.com/api/v2/spot/public/symbols', useProxy: true, parse: r => new Set((r?.data || []).filter(it => (String(it.status || '').toLowerCase() !== 'offline') && String(it.quoteCoin || '').toUpperCase() === 'USDT').map(it => String(it.baseCoin || '').toUpperCase())) },
      BYBIT: { url: 'https://api.bybit.com/v5/market/instruments-info?category=spot', useProxy: true, parse: r => new Set(((r?.result?.list) || []).filter(it => String(it.status || '').toLowerCase() === 'trading').map(it => { const m = /^([A-Z0-9]+)USDT$/.exec(String(it.symbol || '')); return m ? m[1].toUpperCase() : null; }).filter(Boolean)) },
    ...
    for (const cex of selectedCex) {
      ...
      if (!source) continue;
      try {
        // REVISI: Gunakan Http.get dengan opsi proxy, bukan _prox manual
        const data = await this.Http.get(source.url, {
          responseType: 'json',
          proxy: source.useProxy
        });
  ...
  async fetchPrices(selectedCex = []) {
    ...
    const priceSources = {
      BINANCE: { url: 'https://data-api.binance.vision/api/v3/ticker/price', useProxy: true, parse: r => buildPriceMap(r, 'symbol', 'price', /USDT$/) },
      ...
      BITGET: { url: 'https://api.bitget.com/api/v2/spot/market/tickers', useProxy: true, parse: r => buildPriceMap(r?.data || [], 'symbol', 'lastPr', /USDT$/) },
      BYBIT: { url: 'https://api.bybit.com/v5/market/tickers?category=spot', useProxy: true, parse: r => buildPriceMap(r?.result?.list || [], 'symbol', 'lastPrice', /USDT$/) },
      ...
      INDODAX: async (chainKey) => {
        ...
        Object.keys(networkMap || {}).forEach(symRaw => {
          ...
          out.push({
            cex: 'INDODAX',
            chain: chainKey,
            nama_koin: sym, // API tidak menyediakan nama lengkap, gunakan simbol
            nama_token: sym,
            sc_token: '', // Biarkan kosong, akan diisi oleh sync-tab.js dari cache
            des_token: 0, // Set 0 agar sync-tab.js mencoba fetch desimal
