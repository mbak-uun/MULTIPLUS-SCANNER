...
const themeMixin = {
  ...
  methods: {
    ...
    /**
     * REFACTORED: Sistem pewarnaan yang seragam untuk semua komponen
     *
     * PRINSIP PEWARNAAN:
     * 1. BACKGROUND: Semua komponen (card, modal, table, form) menggunakan warna chain theme yang seragam
     *    - Background ditentukan oleh CSS variables: --bg-chain-soft, --bg-chain-accent
     *    - Background TIDAK menggunakan warna dari config CEX/CHAIN/DEX
     *
     * 2. TEKS & BORDER: Warna dari config CEX/CHAIN/DEX hanya digunakan untuk:
     *    - Warna teks (variant='text')
     *    - Warna border (variant='outline')
     *    - Badge solid dengan background warna entitas (variant='solid')
     *
     * 3. VARIANTS:
     *    - 'text': Hanya warna teks dari entitas
     *    - 'outline': Border dan teks dari entitas, background transparan
     *    - 'solid': Background penuh dengan warna entitas (untuk badge CEX/CHAIN/DEX)
     *    - 'soft': Background dari theme chain, border dan teks dari entitas
     *
     * @param {string} entityType - 'cex', 'chain', atau 'dex'
     * @param {string} key - Key dari entitas (misal: 'binance', 'bsc', 'kyber')
     * @param {string} variant - 'text', 'outline', 'solid', atau 'soft' (default)
     * @returns {object} Style object dengan backgroundColor, color, borderColor
     */
    getColorStyles(entityType, key, variant = 'solid') {
      ...
      // Untuk varian 'solid', gunakan warna dari entitas itu sendiri (untuk badge)
      // INI ADALAH SATU-SATUNYA CASE dimana warna CEX/CHAIN/DEX digunakan untuk background
      if (variant === 'solid') {
        return {
          backgroundColor: entityColorInfo.color,
          color: entityColorInfo.contrast,
          borderColor: entityColorInfo.color
        };
      }

      // Default ke varian 'soft' (background dari theme, teks menggunakan warna utama untuk kontras)
      return {
        backgroundColor: 'var(--bg-chain-soft)',
        borderColor: themeColorInfo.color,
        color: 'var(--text-primary)', // Gunakan warna teks utama untuk kontras yang baik
        borderWidth: '1px'
      };
    },
    ...
    updateThemeColor() {
      ...
      // Background colors with chain theme - REFACTORED: Transparansi lebih halus untuk liquid glass effect
      root.style.setProperty('--bg-chain-primary', `rgba(${brandRgb}, 0.04)`);
      root.style.setProperty('--bg-chain-secondary', `rgba(${brandRgb}, 0.06)`);
      root.style.setProperty('--bg-chain-soft', `rgba(${brandRgb}, 0.04)`);
      root.style.setProperty('--bg-chain-accent', `rgba(${brandRgb}, 0.08)`);
