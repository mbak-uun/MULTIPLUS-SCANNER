...
class Http {
  static async request(options = {}) {
    ...
    if (proxy) {
      let proxyUrl = '';
      if (proxy === true) {
        // Use a random proxy from the global config_app
        // REVISI: Menggunakan KONFIG_APLIKASI.LIST_PROXY.SERVERS sesuai struktur config_app.js
        if (typeof KONFIG_APLIKASI !== 'undefined' && KONFIG_APLIKASI.LIST_PROXY && Array.isArray(KONFIG_APLIKASI.LIST_PROXY.SERVERS) && KONFIG_APLIKASI.LIST_PROXY.SERVERS.length > 0) {
          const proxies = KONFIG_APLIKASI.LIST_PROXY.SERVERS;
      ...
      url = proxyUrl ? `${proxyUrl}${encodeURIComponent(originalUrl)}` : originalUrl;
    }
