    ...
    async loadFilterSettings(chainKey) {
      try {
        ...
        if (this.filterSettings.darkMode !== undefined) {
          ...
          // Simpan per-chain dan global fallback
          localStorage.setItem('darkMode_' + chainKey, theme);
          localStorage.setItem('darkMode', theme); // Fallback global

          console.log(`[Theme Sync] Saved theme to localStorage: ${theme} for chain: ${chainKey}`);
        }
