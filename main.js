...
$("#startSCAN").click(function () {
        ...
        // === GLOBAL SCANNING LOCK CHECK (CROSS-TAB) ===
        try {
            const globalLock = getFromLocalStorage('GLOBAL_SCAN_LOCK', null);
            if (globalLock && globalLock.isScanning) {
                const lockedChain = globalLock.chain || 'UNKNOWN';
                const lockedTime = globalLock.timestamp ? new Date(globalLock.timestamp).toLocaleTimeString() : 'unknown time';
                $('#startSCAN').prop('disabled', true).attr('aria-busy','true').text('Locked').addClass('uk-button-disabled');
                if (typeof toast !== 'undefined' && toast.warning) {
                    toast.warning(`Scanning sedang berjalan untuk chain ${lockedChain} (dimulai ${lockedTime}). Hanya 1 scanning aktif diperbolehkan.`);
                } else {
                    alert(`Scanning sedang berjalan untuk chain ${lockedChain}. Hanya 1 scanning aktif diperbolehkan.`);
                }
                return; // prevent starting
            }
        } catch(e) {
            console.error('[GLOBAL LOCK CHECK] Error:', e);
        }
...
$(document).ready(function() {
    ...
    // === CHECK GLOBAL SCAN LOCK ON PAGE LOAD ===
    try {
        const globalLock = getFromLocalStorage('GLOBAL_SCAN_LOCK', null);
        if (globalLock && globalLock.isScanning) {
            // Lock is active: disable START button
            $('#startSCAN').prop('disabled', true).attr('aria-busy','true').text('Locked').addClass('uk-button-disabled');
           // console.log(`[GLOBAL LOCK INIT] Scanning aktif untuk chain: ${globalLock.chain || 'UNKNOWN'}`);
        }
    } catch(e) {
        console.error('[GLOBAL LOCK INIT] Error checking lock:', e);
    }
        ...
        if (window.whenStorageReady && typeof window.whenStorageReady.then === 'function') {
            window.whenStorageReady.then(() => {
                try {
                    ...
                    // Re-check global lock after storage ready
                    const globalLock = getFromLocalStorage('GLOBAL_SCAN_LOCK', null);
                    if (globalLock && globalLock.isScanning) {
                        $('#startSCAN').prop('disabled', true).attr('aria-busy','true').text('Locked').addClass('uk-button-disabled');
                       // console.log(`[GLOBAL LOCK INIT-DELAYED] Scanning aktif untuk chain: ${globalLock.chain || 'UNKNOWN'}`);
                    }
                } catch(_) {}
            });
        }
        ...
        window.__MC_BC.addEventListener('message', function(ev){
            ...
            if (msg.type === 'kv') {
                try {
                    ...
                    // === HANDLE GLOBAL_SCAN_LOCK CHANGES (CROSS-TAB SYNC) ===
                    if (keyUpper === 'GLOBAL_SCAN_LOCK') {
                        const lockData = msg.val || {};
                        if (lockData.isScanning) {
                            // Lock is active: disable START button in all tabs
                            $('#startSCAN').prop('disabled', true).attr('aria-busy','true').text('Locked').addClass('uk-button-disabled');
                           // console.log(`[GLOBAL LOCK SYNC] Lock detected dari tab lain untuk chain: ${lockData.chain || 'UNKNOWN'}`);
                        } else {
                            // Lock released: enable START button if not running locally
                            const localRunning = (typeof window.App?.Scanner?.isScanRunning !== 'undefined') ? !!window.App.Scanner.isScanRunning : false;
                            if (!localRunning) {
                                $('#startSCAN').prop('disabled', false).removeAttr('aria-busy').text('START').removeClass('uk-button-disabled');
                              //  console.log('[GLOBAL LOCK SYNC] Lock released dari tab lain');
                            }
                        }
                        return;
                    }
